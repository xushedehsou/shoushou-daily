...
一个本地优先的个人活动日报：读取前台应用与窗口标题、定时截图、Codex / Claude Code 本地会话、TickTick 只读快照，再把原始材料加工成一页简洁的日报。
...
- TickTick：只读本地数据库，读取当天新增、完成和当前未完成事项；口语标题会先改写成明确任务。
- 总活动时长：只显示“采样估算”，不冒充精确工时。

Bilibili 收藏夹是独立的可选扩展，不属于核心日报链路；本仓库 Demo 不展示收藏夹数据。
