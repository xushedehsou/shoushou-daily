...
读取 ${activity_timeline_file}、${ROOT}/raw/${day}/activity.jsonl、${ai_activity_file}、${screenshot_evidence_file} 和 ${reminders_file}（缺失或读取失败就跳过）。本次任务已经把当天均匀抽取的截图作为图像附件传入；必须先看这些截图，再结合结构化活动记录、会话和窗口记录判断实际工作。会话、截图和绘画记录都只是原始材料，不能原文抄写，必须先加工成客观、明确、正式的工作描述，再写入 ${report_file}。
...
## Timeline
- 按时间顺序列关键工作和生活节点，最多 12 条。每条必须保留可追溯信息，严格使用格式：「HH:MM–HH:MM｜应用：应用名｜项目：项目名或生活｜完成的工作、浏览或形成的产出｜时长：X 小时 Y 分钟｜来源：窗口活动记录 / 本地会话 / 截图识别」。应用名、时间段和来源必须来自结构化活动记录或截图；没有证据就不要编造。窗口标题只能证明看到过窗口，不能写成已经完成了里面的工作。截图识别出的联系人、消息摘要或网页内容要明确写“截图识别”；看不清就只写应用活动，不要猜。
...
## Next
- 只记录材料中明确出现的后续事项。把口语化标题改写为正式、明确、可执行的任务；不要凭空安排任务；没有就写“无”。
